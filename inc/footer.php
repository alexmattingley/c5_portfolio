<footer class="clearfix">
			<ul class="list-unstyled col-xs-12">
				<li class="contact-li col-xs-10 col-xs-offset-1 col-sm-4 col-sm-offset-0 col-lg-3 col-lg-offset-1">
					<h4>Contact</h4>
					<a class="p_Num" href="tel:949.280.6557"></a>
					<a class="email_add" href="mailto:alexmattingley@gmail.com"></a>
				</li>
				<li class="col-xs-10 col-xs-offset-1 col-sm-4 col-sm-offset-0 col-lg-3 col-lg-offset-1">
					<h4>Connect</h4>
					<a href="https://github.com/alexmattingley"><i class="fa fa-github fa-2x"></i></a>
					<a href="https://www.linkedin.com/pub/alex-mattingley/22/27a/8b6"></a><i class="fa fa-linkedin-square fa-2x"></i>
				</li>
				<li class="col-xs-10 col-xs-offset-1 col-sm-4 col-sm-offset-0 col-lg-3 col-lg-offset-1">
					<h4>Location</h4>
					<address>
						<a href="https://www.google.com/maps/place/1501+Tahiti+Ave,+Laguna+Beach,+CA+92651/@33.5472309,-117.769936,17z/data=!3m1!4b1!4m2!3m1!1s0x80dce5c78356c473:0xcd4795846c860057">
						  1501 Tahiti Ave<br>
						  Laguna Beach, CA 92651<br>
						</a>
					</address>
				</li>
			</ul>
		</footer>
		<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script>
		<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/js/bootstrap.min.js"></script>
		<script src="main.js"></script>
	</body>
</html>